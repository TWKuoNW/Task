...
out = cv2.VideoWriter('D:/video/demo.avi', fourcc, 20.0, (1366, 768))
...
while(cap.isOpened()):
    ...
    for result in results:
        ...
        formatted_data = [[int(value) if idx != 4 else round(value, 3) for idx, value in enumerate(inner_list)] for inner_list in data]
        
        for i in range(len(formatted_data)):
            #print(formatted_data[i])
            
            x_min, y_min, x_max, y_max, _, cls_id = map(int, formatted_data[i])
            conf = formatted_data[i][4]
            centroid = ((x_min + x_max) // 2, y_max)  # 計算物件的中心點

            cv2.putText(frame, f'{(conf*100):.2f}%', (x_min, y_min-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1) # 在物件上顯示可信度
            cv2.rectangle(frame, (x_min, y_min), (x_max, y_max), (0, 255, 0), 1)  # 在人物周圍畫矩形框
